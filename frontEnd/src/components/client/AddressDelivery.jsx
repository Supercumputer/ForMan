...
import { createAddress, getAllAddresses, updateAddress } from '../../apis/addressApi';
...
function AddressDelivery() {
    ...
    const onSubmit = async (data) => {
        try {
            const res = await (
                idAddress ? updateAddress(idAddress, data) :
                    createAddress({ ...data, user_id: account?.id }));
    ...
    const callApiGetAllAddress = async () => {
        try {
            const res = await getAllAddresses(account?.id);
            if (res && res.status) {
        ...
        <div>
            <div className='flex justify-between items-center border-b pb-3'>
                ...
                <Button size="md" isProcessing={false} color="dark" onClick={() => setOpenModal(true)}>
                    <div className="flex items-center gap-2"><i className="fa-solid fa-plus"></i><span>Create new address</span></div>
                </Button>
            </div>
