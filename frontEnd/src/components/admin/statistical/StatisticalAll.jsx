...
import { statisticsBestSeller } from '../../../apis/orderApi';
import { getAllVariants } from '../../../apis/variantApi';
function StatisticalAll({ active }) {
    ...
    const callApiGetAllVariant = async () => {
        try {
            const res = await (active === "banchay" ? statisticsBestSeller(`?limit=${limit}`) : getAllVariants(`?limit=${limit}&page=${currentPage}&type=${active}&sort=${active === 'tonkho' ? 'desc' : 'asc'}`));
            if (res && res.status) {
