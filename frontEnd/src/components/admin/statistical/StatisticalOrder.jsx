...
import { getAllOrdersStatistic } from '../../../apis/orderApi';
...
function StatisticalOrder({ active }) {
    ...
    const callApiGetAllOrders = async (currentPage, limit, active, startDate, endDate) => {
        try {
            const res = await getAllOrdersStatistic(`?limit=${limit}&page=${currentPage}&status=${active}&startDate=${startDate}&endDate=${endDate}`);
    ...
    return (
        <div className="rounded-md p-2 bg-white dark:bg-slate-800">
            <div className='flex items-center justify-between mb-2'>
                ...
                <ExportCsv csvData={data} fileName="products" />
            </div>
