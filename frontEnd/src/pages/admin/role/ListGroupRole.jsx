...
function ListGroupRole() {
  ...
  const callApiGetAllGroupRole = async () => {
    try {
      const res = await apiGetAllGroupRole("");
  ...
  const handlerSubmit = async () => {
    try {
      ...
      const res = id
        ? await apiUpdateGroupRole(id, data)
        : await apiGetCreateGroupRole(data);
  ...
  const callApiGetGroupRole = async (id) => {
    try {
      const res = await apiGetGroupRole(id);
      if (res) {
        setId(id);
  ...
  const handlerDelete = async (id) => {
    try {
      const res = await apiDeleteGroupRole(id);
      if (res && res.status) {
        toast.success(res.message);
        callApiGetAllGroupRole();
      } else {
            ...
            <div>
              ...
              {value?.dataCheckValue.map((item, index) => (
                <span
                  ...
                  class="inline-flex items-center px-2 py-1 me-2 mb-2 text-sm font-medium text-blue-800 bg-blue-100 rounded dark:bg-blue-900 dark:text-blue-300"
                >
                  {item.route}
                  <button
